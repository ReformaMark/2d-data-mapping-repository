          ...
          filteredFarms.map((farm, index) => (
            <Card key={index} className="shadow-lg bg-white">
                  ...
                  <span className="font-normal capitalize">
                    {farm.cropHistory.map((crop, index) => (
                      <span key={index} className="inline-flex items-center mr-2">
                        {crop?.name === 'corn' && <Image src={corn.src} alt="Corn" className="inline-block w-6 h-6 mr-1" />}
                        {crop?.name === 'rice' && <Image src={rice.src} alt="Rice" className="inline-block w-6 h-6 mr-1" />}
                        {crop?.name === 'carrots' && <Image src={carrot.src} alt="Carrot" className="inline-block w-6 h-6 mr-1" />}
                        {crop?.name === 'tomatoes' && <Image src={tomatoes.src} alt="Tomatoes" className="inline-block w-6 h-6 mr-1" />}
                        {crop?.name === 'eggplant' && <Image src={eggplant.src} alt="Eggplant" className="inline-block w-6 h-6 mr-1" />}
              ...
              <CardFooter className="bg-gray-50 flex justify-end items-center">
                <Link href={`/stakeholder/farms/${farm._id}`} className="text-blue-500 hover:underline">See More Details</Link>
              </CardFooter>
            </Card>
          ))
        ) : (
