  ...
  return (
    <Card className="p-0 ">
      <CardHeader className="p-0">
        <CardTitle className="bg-gray-100 text-green-500 flex items-center justify-center">
          {" "}
          <Megaphone className="size-10" /> Notifications
        </CardTitle>
      </CardHeader>
      <CardContent className="px-5 py-10 space-y-10">
        <Card className="relative border border-gray-200 shadow-md pt-8 rounded-md px-5">
          <h3 className="absolute top-[-10px] left-3 p-1 font-semibold text-white rounded-md bg-green-500">
            Announcements
          </h3>
          {announcements && announcements.length > 0 ? (
            announcements.slice(0, 5).map((announcement) => (
              <Link
                ...
                className="mb-4 p-2 hover:bg-gray-50 "
              >
                <div className="">
        ...
        <Card className="relative border border-gray-200 shadow-md pt-8 rounded-md px-5">
          ...
          {activities && activities.length > 0 ? (
            activities.slice(0, 5).map((activity) => (
              <Link
                href={"/stakeholder/farms"}
                key={activity._id}
                className="mb-4 p-2 hover:bg-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center"
              >
                ...
                <div className="flex-shrink-0 mt-2 md:mt-0">
                  <h3 className="text-xs">
                    {formatDate({ convexDate: activity._creationTime })}
                  </h3>
                </div>
              </Link>
            ))
